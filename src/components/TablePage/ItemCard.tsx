...
const ItemCard: React.FC<ItemCardProps> = ({ items, dropZoneRef, onDropItem }) => {
    ...
    >
      {items.map((item) =>
        item.is_available && (
          <div
            key={item.id}
            className={`w-full ${expanded.includes(item.id) ? "h-[200px]" : "h-[160px]"
          ...
          >
            ...
            <div className="w-auto h-full flex flex-col">
              ...
              <div className="font-extrabold text-[18px] leading-[24px] pt-1">{item.name}</div>
              ...
              >
                ...
                <div
                  className={`flex mt-[2px] items-center transition-transform duration-500 justify-center ${expanded.includes(item.id) && "rotate-180"
                    }`}
                >
              ...
              <div
                className={`max-w-[200px] overflow-hidden text-[12px] font-bold italic text-gray-400 pt-1.5 ${expanded.includes(item.id) ? "h-[60px] delay-500" : "h-0 delay-0"
                  }`}
              >
                {item.description}
              </div>
      ...
      {draggingItem && (
        ...
        >
          ...
          <div className="text-[12px] text-gray-500">&#x20B9;{draggingItem.price}</div>
        </div>
      )}
    </div>
  );
